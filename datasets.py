...
def _compute_text_similarity_edges(gene_text_emb: np.ndarray,
    ...
    if text_knn > 0:
        ...
        for i in range(0, N, chunk_size):
            ...
            k_actual = min(text_knn, N - 1)  # Don't exceed available neighbors
        ...
        for i in range(0, N, chunk_size):
            ...
            # Compute similarity with remaining genes (to avoid duplicates, only compute upper triangle)
            for j in range(i, N, chunk_size):
                end_j = min(j + chunk_size, N)
                target_emb = embeddings_norm[j:end_j]

                sim_chunk = chunk_emb @ target_emb.T

                # Find edges above threshold
                if i == j:
                    # Same chunk - handle diagonal and upper triangle
                    rows, cols = np.where(sim_chunk >= text_sim_tau)
                    for r, c in zip(rows, cols):
                        global_r, global_c = i + r, j + c
                        if global_r != global_c:  # Exclude self-loops
                            # Add both directions for directed graph
                            text_edges.append([global_r, global_c, sim_chunk[r, c]])
                            text_edges.append([global_c, global_r, sim_chunk[r, c]])
                else:
                    # Different chunks - add all qualifying edges in both directions
                    rows, cols = np.where(sim_chunk >= text_sim_tau)
                    for r, c in zip(rows, cols):
                        global_r, global_c = i + r, j + c
                        text_edges.append([global_r, global_c, sim_chunk[r, c]])
...
def mouse_hsc_nestorowa16(fpath: Optional[str] = './data_cache/mouse_hsc_nestorowa16_v0.h5ad',
